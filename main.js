...
const fileLoctaion = "./assets/toolhead.glb";
const hdriLocation = "./assets/hdri/bg.hdr";
...
const hardwareMaterials = {
    blackOxide: new THREE.MeshStandardMaterial({
        color: 0x1a1a1a,
        metalness: 0.85,
        roughness: 0.4,
    }),
    ...
    blackNickel: new THREE.MeshStandardMaterial({
        color: 0x222b36,
        metalness: 0.9,
        roughness: 0.25,
    })
...
function applyHardwareMaterial(material) {
    if (!model) return;
    model.traverse((obj) => {
        if (obj.isMesh && classifyByComponent(obj).startsWith("hardware")) {
...
function setActiveVersion(v) {
    ...
    const offsetMeters =
        vNorm === "hf" ? -mm(8.5) : vNorm === "uhf" ? -mm(17) : 0;

    model.traverse((o) => {
        if (!o.isMesh) return;
        const cls = classifyByComponent(o);
        if (cls.includes("version")) 
        {
...
function loadModel(url) {
    ...
    loader.load(
        ...
        (gltf) => {
            ...
            const defaultKey = document.querySelector(
                'input[name="hardwareColor"]:checked'
            )?.dataset.key;
...
function classifyByComponent(objOrName) {
    ...
    if (has(tokens, "fasteners"))
        { 
         if (tokens.some((t) => t.startsWith("probescrew")))
         {
            return "hardware-version"; // special case for m3x20 for use in HF, UHF, SF changes on the probe mount
         }
         return "hardware"; // generic hardware bucket
        }
    ...
    const isSF =
        ...
        (has(tokens, "sf") ||
            has(tokens, "standard") ||
            (has(tokens, "beacon") && has(tokens, "mount")));
    ...
    if (hasAny(tokens, ["fan", "fans"])) {
        if (
            tokens.includes("blower") ||
            tokens.some((t) => t.startsWith("blower"))
        ) {
            return "fan-version"; // special class for blower fans
        }
    ...
    if (has(tokens, "heatset")) {
         if (tokens.some((t) => t.startsWith("ductheatset")))
         {
            return "heatset-version"; // special case for m3x20 for use in HF, UHF, SF changes on the probe mount
         }
        return "heatset";
    }
    if (has(tokens, "thumbscrew"))
        return "thumbscrew"; // generic thumbscrew 
    if (has(tokens, "spring"))
        return "spring"; // generic spring
    if (hasAny(tokens, ["belt", "belts"]))
        return "belt"; // generic belt
    ...
    model.traverse((obj) => {
        ...
        switch (cls) {
            ...
            case "motor":
                ...
                break;
            case "heatset":
            case "heatset-version": 
                chosen = new THREE.Color(0xdaa520);
            break;
            case "thumbscrew":
                chosen = new THREE.Color(0x444444);
                break;
            case "spring":
                chosen = new THREE.Color(0xCCCCCC); 
                break;
            case "belt":
                chosen = new THREE.Color(0x5c2d26);
                break;
            default:
                return;
        }
...
initScene();
initLighting();
loadEnvironmentMap(hdriLocation);
bindHardwareRadioButtons();
addVersionDropdown();
loadModel(fileLoctaion);
resize();
...
document.getElementById("screenshotFab")?.addEventListener("click", () => {
    try {
        ...
        a.href = imageDataURL;
        a.download = `ChromaColony-${new Date()
            .toISOString()
            .replace(/[:.]/g, "-")}.png`;
